...
describe("session frame.append -> adapter wiring", () => {
  // Per-frame upstream forwarding of axText is currently OFF — sending
  // realtimeInput.text on every video frame triggers a Gemini tool-call
  // storm + 1007. These tests guard the OFF state until a turn-bounded
  // forwarding strategy lands.
  it("never calls sendAxText on adapter, even with axText present", async () => {
  ...
  it("still sends the video frame regardless of axText state", async () => {
    const { session, spy } = makeSessionWithSpyAdapter()
  ...
  it("rejects non-string axText at the trust boundary (no upstream leak)", async () => {
    const { session, spy } = makeSessionWithSpyAdapter()
    await deliver(session, {
      type: "frame.append",
      data: "<jpeg>",
      axText: { malicious: "object" } as unknown as string,
    })
    expect(spy.frames).toHaveLength(1)
    expect(spy.axTexts).toEqual([])
  })
