...
DMG installs of the VoiceClaw desktop app ship a precompiled relay binary under `Contents/Resources/bin/relay-server-darwin-arm64` and spawn it automatically as a managed service alongside `openclaw-gateway`. End users do not need to run `yarn dev` or install Node — the bundled binary boots on first launch and listens on port `8080` (with kernel-allocated fallback if the port is already taken).

The binary is produced by `yarn workspace relay-server build:bin`, which uses [Bun's `--compile` mode](https://bun.sh/docs/bundler/executables) to package the relay's pure-JS dependencies (`express`, `ws`, OpenTelemetry SDKs, `dotenv`) into a single ~59 MB Mach-O executable. The signed and notarized binary inherits the desktop app's hardened-runtime entitlements.

In development you continue to run the relay via `concurrently` from the repo root (`yarn dev`); the desktop app's bundled-service spawn is a no-op when the binary is absent, so the renderer keeps talking to the locally tsx-watched relay.

<Aside type="caution">
Initial release ships **arm64 only**. Intel Macs hit the "binary not found" branch and the desktop app continues without a bundled relay, requiring users to point at an external relay endpoint via Settings. A `relay-server-darwin-x64` build (and `lipo`-merged universal binary) is tracked as a follow-up under NAN-681.
</Aside>
