...
// A sidecar that runs uninterrupted for at least this long is considered
// "stable" — we reset the restart counter so transient OS hiccups don't
// permanently exhaust the retry budget. Without this gate, a sidecar that
// crashes immediately after spawn would loop forever at the smallest
// backoff because each new spawn reset attempts to zero.
const RESTART_STABLE_RUNTIME_MS = 30 * 1000
...
function startSidecar() {
  ...
  // Don't reset restartAttempts yet — we only know the sidecar was healthy
  // once it has been alive for RESTART_STABLE_RUNTIME_MS. Resetting on
  // spawn lets a crash-loop forever-retry at the minimum backoff.
  ...
  proc.on('exit', (code, signal) => {
    ...
    proc = null
    failAllPending()
    if (ranFor >= RESTART_STABLE_RUNTIME_MS) restartAttempts = 0
...
function scheduleRestart() {
  ...
  setTimeout(() => {
    if (!proc && !shuttingDown) startSidecar()
  }, delay)
}
