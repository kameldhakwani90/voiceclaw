...
export class ScreenCapture {
  ...
  async start(
    ...
    // AX capture is only safe for full-display sources. When sharing a
    // specific window the captured pixels follow that window even when it's
    // not focused, but AX text is sourced from whichever window is *frontmost*
    // — those can disagree, leaking text from an unrelated window into the
    // model's view. For window-source shares we ship vision-only.
    const sourceIsDisplay = sourceId.startsWith('screen:')
    this.axEnabled = (options?.axEnabled ?? true) && sourceIsDisplay
    this.stopped = false
  ...
  private async captureFrame(onFrame: (frame: ScreenFrame) => void) {
    ...
    // Re-check stop() — captureAxTextOrTimeout awaits, and the user may have
    // ended sharing in that window. Without this, a single stale frame can
    // land after stop().
